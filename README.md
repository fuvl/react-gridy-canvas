...
| `onLayoutChange`        | `(newLayout: GridItem[], oldLayout: GridItem[]) => void`                                                                                                           | _Required_                    | Callback function triggered when the layout changes. Receives the new layout array and the layout array before the change.                     |
...
| `snapGridUnit`          | `number`                                                                                                                                                           | `10`                          | The size of the grid unit (in pixels) for snapping item positions during drag.                                                                 |
| `resizeGridUnit`        | `number`                                                                                                                                                           | `10`                          | The size of the grid unit (in pixels) for snapping item dimensions during resize.                                                              |
...
| `getItemClassName`      | `(itemId: string) => string`                                                                                                                                       | `undefined`                   | Optional function that receives an item's ID and returns a string of CSS class names to apply to that specific grid item container.            |
